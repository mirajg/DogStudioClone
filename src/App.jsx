...
const App = () => {
  ...
  // Use a Ref to track scroll state without triggering re-renders
  const didScrollDown = useRef(false);
  const sec3 = useRef(null);
  const navRef = useRef(null);
  const [imgSelect, setImgSelect] = useState("/pictures/mat-19.png");

  const [bgStyle, setBgStyle] = useState({
    background: `url(/pictures/background-1.png)`,
    backgroundColor: 'transparent'
  });

  useEffect(() => {

    sec3.current.addEventListener("mouseenter", () => {
      // Use the ID instead of the Canvas Ref to ensure you hit the DOM
      const canvasDOM = document.getElementById('canvas');
      if (canvasDOM) {
        canvasDOM.style.background = "url('/pictures/background-2.png')";
        canvasDOM.style.backgroundSize = "cover";
      }
      setImgSelect("/pictures/mat-1.png");
    });
    sec3.current.addEventListener("mouseleave", () => {
      // Use the ID instead of the Canvas Ref to ensure you hit the DOM
      const canvasDOM = document.getElementById('canvas');
      setImgSelect("/pictures/mat-19.png");
      if (canvasDOM) {
        canvasDOM.style.background = "url('/pictures/background-1.png')";
        canvasDOM.style.backgroundColor = "transparent";
        canvasDOM.style.backgroundSize = "cover";
      }
    });
    // }
    ...
    const handleScroll = () => {
      const scrollTop = window.scrollY; // Modern alternative to pageYOffset

      if (scrollTop > 100) {
        didScrollDown.current = true;
      }

      // If at top AND we previously scrolled down
    ...
    <>
      <main>
        <Canvas id='canvas' style={{
          ...
          <Dog fetchImg={imgSelect} sec3={sec3} />
        </Canvas>

        <section id="section-1">
          <nav ref={navRef} className={`navbar ${isBouncing ? 'bounce-animation' : ''}`}>
            <ul style={{
              display: "flex",
              ...
              <li><svg width={100} fill="white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 401.23099 116.838">
          ...
          <div className='section-1-div-wrapper'>
            <div className='sideSection1'>
              <span>We</span>
              <span>Make</span>
              <span>Good</span>
              <span>Shit</span></div>
            <div className='sideSection2'></div>
          </div>
          <div className='rotated-line'></div>
          <div className='info-wrapper'>
            <div className='info-wrapper1'></div>
            <div className='info-wrapper2'>
              <div className='first-info-wrapper1'>Dogstudio is a multidisciplinary
                creative studio at the intersection
                of art, design and technology.</div>
              <div style={{ color: "grey" }}>Our goal is to deliver amazing experiences that make
                people talk, and build strategic value for brands, tech,
                entertainment, arts & culture.</div>
            </div>
          </div>
        </section>
        <section id="section-2"></section>
        <section ref={sec3} id="section-3">
          <div className="top">
            <div className="left">Lorem ipsum dolor sit amet consectetur adipisicing elit. Ipsa odit voluptatem error eligendi perspiciatis odio placeat eum repudiandae incidunt eaque aperiam ab fugit inventore laborum corporis, et recusandae aliquid ullam minus eius dolores fugiat suscipit. Odit eum fuga ratione deserunt commodi dolor repellat, delectus asperiores sed quas at obcaecati, neque eveniet suscipit facere doloribus aliquid atque vero voluptas autem! Ratione illo id ex, quia consectetur error voluptate in velit assumenda nobis quod ullam adipisci, itaque impedit praesentium similique? Dolorem illum, ipsam facilis numquam laboriosam quo delectus, nesciunt, provident aperiam mollitia nam sunt! Quisquam ipsam pariatur soluta hic accusantium tenetur omnis.</div>
        ...
        <div id="section-4">
          <div className="btm">
            ...
            <div className="right">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Inventore cumque molestias harum odit blanditiis voluptate? Dolorem optio ex eum animi deleniti. Tenetur nemo blanditiis aliquid officiis dignissimos quis labore consequuntur nostrum unde quae reprehenderit libero id voluptate eligendi quia quaerat laborum aperiam odio veniam officia, impedit provident doloribus commodi. Adipisci sunt nesciunt, ipsa numquam eius dignissimos praesentium nostrum perspiciatis suscipit autem, eum unde architecto consectetur eos? Similique, nesciunt recusandae? Atque ipsa quibusdam earum nobis. Libero, ea, eaque esse cupiditate laudantium molestiae voluptatum praesentium quaerat harum illum sunt earum voluptas quis vero nobis sapiente saepe non deleniti aperiam assumenda? Totam, eaque.</div>
          </div>
        </div>
      </main>
    </>
